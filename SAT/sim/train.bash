...
temp_script="scripts/temp/train_policy_temp_${timestamp}.sh"

if [ -f "scripts/train_policy.sh" ]; then
  cp "scripts/train_policy.sh" "$temp_script"
  echo "created copy : $temp_script"
else
  echo "ERROR: scripts/train_policy.sh not found"
  exit 1
fi

# Optional: Start proxy if needed
# (
#   cd /path/to/proxy || exit 1
#   ./proxy-command &
# )
# sleep 0.1
